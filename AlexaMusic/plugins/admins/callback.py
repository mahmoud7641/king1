...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        check = db.get(chat_id)
        txt = f"» تم التخطي بواسطة {mention} !"
        popped = None
        try:
            popped = check.pop(0)
            if popped:
            ...
            if not check:
                await CallbackQuery.edit_message_text(f"» تم التخطي بواسطة  {mention} !")
                await CallbackQuery.message.reply_text(
        ...
        if int(command) in [1, 3]:
            if (duration_played - duration_to_skip) <= 10:
                ...
                return await CallbackQuery.answer(
                    f"» البوت غير قادر على البحث لأن الوقت يتجاوز المدة المسموح بها.\n\nالتشغيل حالياً :** {bet}** دقائق من**{duration}** دقيقة.",
                    show_alert=True,
                )
