...
async def ping_com(client, message: Message, _):
    ...
    if "live_" in file:
        ...
        if videoid == "telegram":
            ...
            IMAGE = get_image(videoid)
    send = (
        "**⌛️المدة:**  مدة البث غير معروفة\n\nاضغط على الزر أدناه للحصول علي قائمة الانتظار بأكملها."
        if DUR == "Unknown"
        else "\nاضغط على الزر أدناه للحصول علي قائمة الانتظار بأكملها."
    )
    cap = f"""**{config.MUSIC_BOT_NAME} المشغل**
